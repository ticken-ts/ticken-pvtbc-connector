...
func (cc DevChaincodeAPI) handleEventCCAPI(name string, args ...string) ([]byte, uuid.UUID, *client.ChaincodeEvent, error) {
	...
	case consts.EventCCSellFunction:
		return cc.handleEventCCSellTx(args...)
	case consts.EventCCStartFunction:
		return cc.handleEventCCStartTx(args...)
	case consts.EventCCFinishFunction:
		return cc.handleEventCCFinishTx(args...)
...
func (cc DevChaincodeAPI) handleEventCCSellTx(args ...string) ([]byte, uuid.UUID, *client.ChaincodeEvent, error) {
	if len(args) != 1 {
		return nil, uuid.Nil, nil, fmt.Errorf("wrong arg numbers: expected %d, obtained %d", 1, len(args))
	}
...
func (cc DevChaincodeAPI) handleEventCCStartTx(args ...string) ([]byte, uuid.UUID, *client.ChaincodeEvent, error) {
	if len(args) != 1 {
		return nil, uuid.Nil, nil, fmt.Errorf("wrong arg numbers: expected %d, obtained %d", 1, len(args))
	}
...
func (cc DevChaincodeAPI) handleEventCCFinishTx(args ...string) ([]byte, uuid.UUID, *client.ChaincodeEvent, error) {
	if len(args) != 1 {
		return nil, uuid.Nil, nil, fmt.Errorf("wrong arg numbers: expected %d, obtained %d", 1, len(args))
	}
	...
	event.Status = chainmodels.EventStatusFinished
